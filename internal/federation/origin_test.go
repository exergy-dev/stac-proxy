...
func TestShouldRetry(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:        "nil policy - retry 5xx",
			retryPolicy: nil,
			statusCode:  500,
			want:        true,
		},
		{
			name:        "nil policy - retry 502",
			retryPolicy: nil,
			statusCode:  502,
			want:        true,
		},
		{
			name:        "nil policy - do not retry 4xx",
			retryPolicy: nil,
			statusCode:  404,
			want:        false,
		},
		{
			name: "custom retry codes - retry 503",
			retryPolicy: &RetryPolicy{
				RetryOn: []int{502, 503, 504},
			},
			statusCode: 503,
		...
		{
			name: "custom retry codes - do not retry 500",
			retryPolicy: &RetryPolicy{
				RetryOn: []int{502, 503, 504},
			},
			statusCode: 500,
		...
		{
			name: "custom retry codes - do not retry 404",
			retryPolicy: &RetryPolicy{
				RetryOn: []int{502, 503, 504},
			},
			statusCode: 404,
		...
		{
			name: "empty retry codes - default to 5xx",
			retryPolicy: &RetryPolicy{
				RetryOn: []int{},
			},
			statusCode: 500,
