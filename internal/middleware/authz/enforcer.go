...
// not short-circuit (so a transient outage doesn't deny everything when
// another enforcer would have allowed), but they ARE surfaced in the
// final Reasons so operators can diagnose silent fall-throughs.
//
// An enforcer that returns a decision with Final=true is treated as
// authoritative and short-circuits the loop in either direction — an
// authoritative deny cannot fall through to a more permissive fallback
// enforcer.
func (e *CompositeEnforcer) authorizeAny(ctx context.Context, input *AuthzInput) (*AuthzDecision, error) {
	...
	for _, enforcer := range e.enforcers {
		...
		// declared its decision authoritative. Falling through to another
		// enforcer would reintroduce the fail-open the Final marker exists
		// to prevent.
		if decision.Final {
