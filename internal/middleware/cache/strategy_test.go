...
func TestOriginAwareStrategy(t *testing.T) {
	t.Parallel()

	strategy := NewOriginAwareStrategy()

	t.Run("Inherits default strategy TTLs", func(t *testing.T) {
		t.Parallel()
	...
	t.Run("Generate key without origins", func(t *testing.T) {
		t.Parallel()
		...
		baseKey := strategy.DefaultStrategy.GenerateKey(req)

		if key != baseKey {
			t.Error("Key should match base key when no origins present")
		}
	})

	t.Run("Generate key with origins", func(t *testing.T) {
		t.Parallel()
		...
		req := &middleware.STACRequest{
			...
			Params: map[string]interface{}{
				"_origins": []string{"origin1", "origin2"},
			},
		}
		...
		if key == baseKey {
			t.Error("Key should differ from base key when origins present")
		}
	...
	t.Run("Generate key with origins in different order", func(t *testing.T) {
		t.Parallel()
		...
		req2 := &middleware.STACRequest{
			...
			Collection:  "test",
			Params: map[string]interface{}{
				"_origins": []string{"origin2", "origin1"},
			},
		}
		...
		if key1 != key2 {
			t.Error("Keys should be same regardless of origin order")
		}
	})

	t.Run("Generate key with different origins", func(t *testing.T) {
		t.Parallel()
		...
		if key1 == key2 {
			t.Error("Keys should differ for different origins")
		}
	})
}

func TestUserAwareStrategy(t *testing.T) {
	t.Parallel()

	t.Run("Per-user disabled", func(t *testing.T) {
		t.Parallel()

		strategy := NewUserAwareStrategy(false)

		if strategy.PerUser {
			t.Error("PerUser should be false")
		}
		...
		if key != baseKey {
			t.Error("Key should match base key when per-user is disabled")
		}
	})

	t.Run("Per-user enabled without user ID", func(t *testing.T) {
		t.Parallel()

		strategy := NewUserAwareStrategy(true)
		...
		if key != baseKey {
			t.Error("Key should match base key when no user ID present")
		}
	})

	t.Run("Per-user enabled with user ID", func(t *testing.T) {
		t.Parallel()
		...
		if key == baseKey {
			t.Error("Key should differ from base key when user ID present")
		}
	...
	t.Run("Different users get different keys", func(t *testing.T) {
		...
		if key1 == key2 {
			t.Error("Keys should differ for different users")
		}
	})

	t.Run("Empty user ID treated as no user ID", func(t *testing.T) {
		t.Parallel()
		...
		if key != baseKey {
			t.Error("Empty user ID should be treated as no user ID")
		}
	})
}
...
func TestCacheStrategyInterface(t *testing.T) {
	...
	var _ CacheStrategy = (*OriginAwareStrategy)(nil)
	var _ CacheStrategy = (*UserAwareStrategy)(nil)
...
func BenchmarkOriginAwareStrategy_GenerateKey(b *testing.B) {
	strategy := NewOriginAwareStrategy()
	req := &middleware.STACRequest{
		Request:     httptest.NewRequest("GET", "/collections/test", nil),
		RequestType: middleware.RequestTypeCollection,
		Collection:  "test",
		Params: map[string]interface{}{
			"_origins": []string{"origin1", "origin2", "origin3"},
		},
	}
...
func BenchmarkUserAwareStrategy_GenerateKey(b *testing.B) {
	strategy := NewUserAwareStrategy(true)
	req := &middleware.STACRequest{
		Request:     httptest.NewRequest("GET", "/collections/test", nil),
		RequestType: middleware.RequestTypeCollection,
