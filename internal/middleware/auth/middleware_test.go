...
func TestProviderFunc(t *testing.T) {
	...
	fn := ProviderFunc(func(ctx context.Context, req *http.Request) (*Principal, error) {
		called = true
		capturedReq = req
		return &Principal{ID: "func-user", Type: "user"}, nil
	})

	if fn.Name() != "func" {
		t.Errorf("Name() = %q, want 'func'", fn.Name())
	}

	httpReq := httptest.NewRequest("GET", "/", nil)
	ctx := context.Background()

	principal, err := fn.Authenticate(ctx, httpReq)
	if err != nil {
	...
	if capturedReq != httpReq {
		t.Error("ProviderFunc did not receive the correct request")
	}
	...
	if principal.ID != "func-user" {
		t.Errorf("Principal.ID = %q, want 'func-user'", principal.ID)
	}
}
